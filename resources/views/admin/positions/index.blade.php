...
@section('title', __('positions.page_title'))
            ...
            <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                ...
                <div class="shrink-0">
                    <x-ui.button type="button" :full="false"
                        ...
                        <span>{{ __('positions.add_new') }}</span>
                    </x-ui.button>
                </div>
...
<section class="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm">
        ...
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div class="relative w-full xl:max-w-md">
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="{{ __('positions.filters.search_placeholder') }}"
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <select name="status"
                    ...
                    <option value="all">{{ __('positions.filters.all_statuses') }}</option>
    ...
    <div class="hidden overflow-x-auto xl:block">
        <table class="w-full min-w-[900px] text-sm">
            <thead class="bg-[#f8fbff] text-slate-500">
                <tr>
                    <th class="px-5 py-5 text-start font-bold">{{ __('positions.table.id') }}</th>
                    <th class="px-5 py-5 text-start font-bold">{{ __('positions.table.name') }}</th>
                    <th class="px-5 py-5 text-start font-bold">{{ __('positions.table.status') }}</th>
                    <th class="px-5 py-5 text-start font-bold">{{ __('positions.table.created_at') }}</th>
                    <th class="px-5 py-5 text-start font-bold">{{ __('positions.table.updated_at') }}</th>
                    <th class="px-5 py-5 text-start font-bold">{{ __('positions.table.actions') }}</th>
                </tr>
            </thead>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($positions as $position)
                    ...
                    <tr onclick="window.location.href='{{ route('admin.positions.show', $position->id) }}'"
                        ...
                        <td class="relative px-5 py-5" onclick="event.stopPropagation()">
                            <details class="group relative inline-block">
                                ...
                                <div
                                    class="absolute end-0 z-50 mt-2 w-44 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
                                    ...
                                    <a href="{{ route('admin.positions.edit', $position->id) }}"
                                        ...
                                        {{ __('positions.actions.edit') }}
                                    </a>

                                    {{-- Delete --}}
                                    <form method="POST"
                                        action="{{ route('admin.positions.destroy', $position->id) }}"
                                        onsubmit="return confirm('{{ __('positions.actions.confirm_delete') }}')">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="grid gap-4 p-4 xl:hidden">
        @forelse($positions as $position)
            ...
            <div onclick="window.location.href='{{ route('admin.positions.show', $position->id) }}'"
                    ...
                    <div class="flex items-center justify-end gap-2">
                        ...
                        @if ($positions->onFirstPage())
                            <span class="inline-flex h-10 cursor-not-allowed items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-extrabold text-slate-300">
                                {{ __('positions.pagination.previous') }}
                            </span>
                        @else
                        ...
                        <span class="inline-flex h-10 items-center justify-center rounded-xl bg-[#0f1b3d] px-4 text-sm font-extrabold text-white">
                            {{ __('positions.pagination.page') }}
                            {{ $positions->currentPage() }}
                            {{ __('positions.pagination.from') }}
                            {{ $positions->lastPage() }}
                        </span>
