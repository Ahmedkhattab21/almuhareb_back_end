    ...
    <section class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-500">مرتبطة بالتذكرة رقم #{{ $recommendation->ticket_id }}</p>
            </div>

            <a href="{{ route('admin.recommendations.index') }}" class="inline-flex h-12 items-center justify-center rounded-2xl border border-slate-200 px-6 text-sm font-extrabold text-[#0f1b3d]">رجوع</a>
        </div>
    </section>
    ...
    <section class="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-6 flex flex-wrap gap-3">
            @if(Route::has('admin.tickets.show'))
                <a href="{{ route('admin.tickets.show', $recommendation->ticket_id) }}" class="inline-flex h-11 items-center rounded-2xl bg-[#0f1b3d] px-5 text-sm font-extrabold text-white">فتح التذكرة</a>
            @endif
