...
@section('title', __('company_news.admin.page_title'))
...
<div class="space-y-6">
    <section class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <h1 class="mt-2 text-3xl font-black text-[#0f1b3d] sm:text-4xl">{{ __('company_news.admin.title') }}</h1>
            <p class="mt-2 text-sm leading-7 text-slate-500">{{ __('company_news.admin.subtitle') }}</p>
        </div>

        <a href="{{ route('admin.company-news.create') }}"
    ...
    <section class="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm">
        <form method="GET" action="{{ route('admin.company-news.index') }}" class="border-b border-slate-100 p-5">
            <div class="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="{{ __('company_news.filters.search') }}"
                    class="h-12 w-full rounded-2xl border border-slate-200 bg-[#f8fbff] px-4 text-sm font-bold outline-none focus:border-[#5368aa] xl:max-w-xl">

                <div class="flex flex-wrap items-center gap-3">
                    <select name="company_id"
                        class="h-12 min-w-[220px] rounded-2xl border border-slate-200 bg-[#f8fbff] px-4 text-sm font-bold outline-none focus:border-[#5368aa]">
                        <option value="all">{{ __('company_news.filters.all_companies') }}</option>
                    ...
                    <a href="{{ route('admin.company-news.index') }}" class="text-sm font-bold text-blue-700">{{ __('company_news.filters.reset') }}</a>
                </div>
            </div>
        </form>
        ...
        <div class="grid gap-5 p-5 md:grid-cols-2 xl:grid-cols-3">
            @forelse($news as $item)
                <article class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    <a href="{{ route('admin.company-news.show', $item) }}" class="block">
                        @if($item->image_url)
                        ...
                        <div class="space-y-3 p-5">
                            <p class="text-xs font-bold text-slate-500">{{ $item->company?->company_name }}</p>
