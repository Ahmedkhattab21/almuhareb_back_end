...
<div class="space-y-6">
    <section class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            <div class="text-sm text-slate-500">
                {{ __('ratings.breadcrumb_lawyer') }}
                <span class="mx-1">›</span>
                <span class="font-bold text-[#0f1b3d]">{{ __('ratings.lawyer_title') }}</span>
            </div>

            <h1 class="mt-2 text-3xl font-black text-[#0f1b3d]">{{ __('ratings.lawyer_title') }}</h1>
            <p class="mt-2 text-sm leading-7 text-slate-500">{{ __('ratings.lawyer_subtitle') }}</p>
        </div>
    </section>
    ...
    <section class="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm">
        <form method="GET" action="{{ route('lawyer.ratings.index') }}" class="border-b border-slate-100 p-5">
            <div class="flex flex-col gap-3 xl:flex-row xl:items-center">
                <input
                    type="text"
                    name="search"
                    value="{{ request('search') }}"
                    placeholder="{{ __('ratings.lawyer_search_placeholder') }}"
                ...
                <a href="{{ route('lawyer.ratings.index') }}" class="inline-flex h-12 items-center rounded-2xl px-4 text-sm font-bold text-blue-700">
                    {{ __('ratings.reset') }}
                </a>
            </div>
        </form>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[900px] text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($ratings as $rating)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="max-w-sm px-5 py-5 text-slate-600">
                                {{ $rating->message ? \Illuminate\Support\Str::limit($rating->message, 100) : __('ratings.no_message') }}
                            </td>
                            <td class="px-5 py-5 text-slate-500">{{ $rating->created_at?->format('Y-m-d H:i') ?? '-' }}</td>
                            <td class="px-5 py-5">
                                @if($rating->ticket && Route::has('lawyer.tickets.show'))
                                    <a href="{{ route('lawyer.tickets.show', $rating->ticket) }}" class="font-bold text-blue-700 hover:underline">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-14 text-center text-slate-500">{{ __('ratings.empty') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
