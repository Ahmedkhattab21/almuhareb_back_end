...
<div class="space-y-6">
    <section class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="max-w-4xl">
            <p class="text-sm font-bold text-slate-500">{{ __('company_news.title') }}</p>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <a href="{{ route('company.company-news.edit', $newsItem) }}" class="rounded-2xl bg-[#0f1b3d] px-5 py-3 text-sm font-extrabold text-white">
                {{ __('company_news.actions.edit') }}
            </a>
            <form method="POST" action="{{ route('company.company-news.destroy', $newsItem) }}" onsubmit="return confirm('{{ __('company_news.actions.confirm_delete') }}')">
                @csrf
                @method('DELETE')
                <button class="rounded-2xl bg-red-600 px-5 py-3 text-sm font-extrabold text-white">{{ __('company_news.actions.delete') }}</button>
            </form>
            <a href="{{ route('company.company-news.index') }}" class="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-bold text-[#0f1b3d]">
    ...
    <article class="overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm">
        ...
        <div class="space-y-6 p-6">
            ...
            <div class="grid gap-4 border-t border-slate-100 pt-5 text-sm sm:grid-cols-2">
                <div>
                    <p class="font-bold text-slate-500">{{ __('company_news.show.created_at') }}</p>
                    <p class="mt-1 font-black text-[#0f1b3d]">{{ $newsItem->created_at?->format('Y-m-d H:i') }}</p>
                </div>
                <div>
